...
```sh
# 設定を適用 (ユーザー設定: home-manager standalone)
nix flake update && git add -A && home-manager switch

# システム設定を適用 (nix-darwin: /Library 等のシステム領域。要 sudo)
# 初回のみ: sudo nix run nix-darwin/master#darwin-rebuild -- switch --flake .#takegawanoMacBook-Pro
sudo darwin-rebuild switch --flake .#takegawanoMacBook-Pro
...
```
flake.nix              # エントリポイント。nixpkgs-unstable + nixpkgs-master + sops-nix + nix-darwin
home.nix               # ユーザー設定 (home-manager standalone)。imports + sops secrets 定義
darwin.nix             # システム設定 (nix-darwin)。home-manager とは並存。要 sudo で適用
...
programs/              # ツール別モジュール (各ディレクトリに default.nix + 関連ファイル)
  ...
  blackhole/           # BlackHole 仮想オーディオドライバ (nix-darwin 専用。HAL ドライバをシステムへ配置)
...
- **Git 運用: このリポジトリは `main` に直接コミットする** (個人の home-manager 設定のため。ブランチ + PR は不要)。commit スキルの main 保護はこのリポジトリでは無視してよい。
